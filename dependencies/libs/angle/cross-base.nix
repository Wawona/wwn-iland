{
  ...
  gnExtraFlags,
  clangBasePath ? null,
  buildTargets ? "angle angle_common libEGL libGLESv2",
  patchesExtra ? [ ],
  preConfigureHook ? "",
  installHook ? "",
  postPatchExtra ? "",
...
let
  ...
  hostTriplet = lib.optionalString pkgs.stdenv.hostPlatform.isLinux (
    lib.getAttr hostArch {
...
pkgs.stdenv.mkDerivation (finalAttrs: {
  ...
  buildInputs = [ pkgs.apple-sdk_15 ] ++ buildInputsExtra;
  ...
  patches = [ anglePatch ] ++ patchesExtra;
